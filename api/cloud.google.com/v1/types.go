...
// +kubebuilder:validation:XValidation:rule="(has(self.nodePoolConfig) && has(self.nodePoolConfig.sandbox) && has(self.nodePoolConfig.sandbox.type) && self.nodePoolConfig.sandbox.type == 'microvm') ? self.priorities.all(p, has(p.enableNestedVirtualization) ? p.enableNestedVirtualization == true : (has(self.priorityDefaults) && has(self.priorityDefaults.enableNestedVirtualization) && self.priorityDefaults.enableNestedVirtualization == true)) : true", message="ComputeClass with sandbox type 'microvm' requires enableNestedVirtualization to be true for all priorities"
