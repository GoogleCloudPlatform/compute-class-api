...
// +kubebuilder:validation:XValidation:rule="(has(self.autopilot) && self.autopilot.enabled) ? ((!has(self.priorityDefaults) || !has(self.priorityDefaults.nodeSystemConfig) || !has(self.priorityDefaults.nodeSystemConfig.linuxNodeConfig) || !has(self.priorityDefaults.nodeSystemConfig.linuxNodeConfig.sysctls) || !has(self.priorityDefaults.nodeSystemConfig.linuxNodeConfig.sysctls.net_ipv4_tcp_congestion_control)) && self.priorities.all(p, !has(p.nodeSystemConfig) || !has(p.nodeSystemConfig.linuxNodeConfig) || !has(p.nodeSystemConfig.linuxNodeConfig.sysctls) || !has(p.nodeSystemConfig.linuxNodeConfig.sysctls.net_ipv4_tcp_congestion_control))) : true", message="Sysctl config net_ipv4_tcp_congestion_control cannot be set when Autopilot is enabled"
...
// +kubebuilder:validation:XValidation:rule="!(has(self.machineFamily) && self.machineFamily == 'e4a')", message="MachineFamily cannot be equal to 'e4a'"
